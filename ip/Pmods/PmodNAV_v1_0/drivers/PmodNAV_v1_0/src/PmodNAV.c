/************************************************************************/
/*                                                                      */
/* PmodNAV.c    --      Driver for the Pmod NAV using SPI               */
/*                                                                      */
/************************************************************************/
...
/*  File Description:                                                   */
/*                                                                      */
/*  This file contains library to be used in XIlinx SDK to interface    */
/*  with the PmodNAV                                                    */
/*                                                                      */
/************************************************************************/
/*  Revision History:                                                   */
...
**      CpuClockFreqHz - frequency of the CPU clock, required for functionality in microblaze
**
**  Return Value:
**      none
**
...
void NAV_begin(PmodNAV* InstancePtr, u32 GPIO_Address, u32 SPI_Address)
{
    ...
    Xil_Out32(InstancePtr->GPIO_addr+4, 0b11);//0b1111 for input 0b0000 for output, 0b0001 for pin1 in pin 2 out etc.
...
int NAV_SPIInit(XSpi *SpiInstancePtr) {
    int Status;
//  XSpi *SpiInstancePtr = &InstancePtr->NAVSpi;

    Status = XSpi_CfgInitialize(SpiInstancePtr, &NAVConfig, NAVConfig.BaseAddress);
    if (Status != XST_SUCCESS)
        return XST_FAILURE;

    Status = XSpi_SetOptions(SpiInstancePtr, XSP_MASTER_OPTION | XSP_CLK_ACTIVE_LOW_OPTION | XSP_CLK_PHASE_1_OPTION);
    if (Status != XST_SUCCESS)
        return XST_FAILURE;
...
**      	set active mode and 7Hz ODR rate, in register NAV_ALT_CTRL_REG1, block data
**      	update active.
**
*/
void NAV_InitALT(PmodNAV* InstancePtr, bool fInit)
...
**          for ACL mode:
**          	Enable all three axes in NAV_ACL_CTRL_REG5_XL register,
**              set 10 Hz ODR in NAV_ACL_CTRL_REG6_XL register
**          for ACL+GYRO:
**              set 10Hz ODR in NAV_GYRO_CTRL_REG1_G register and NAV_ACL_CTRL_REG6_XL, thus enabling the Gyroscope functionality as well;
...
**          10H ODR, in register NAV_MAG_CTRL_REG1_M
**          disable I2C and enable SPI read and write operations,
**          set the operating mode to continuous in NAV_MAG_CTRL_REG3_M register.
**
*/
void NAV_InitMAG(PmodNAV* InstancePtr, bool fInit)
{
...
void NAV_GetDeviceID(PmodNAV* InstancePtr)
{
    NAV_ReadRegister(InstancePtr, NAV_INST_AG, NAV_ACL_GYRO_WHO_AM_I, 1, &(InstancePtr->idData.ag));
    NAV_ReadRegister(InstancePtr, NAV_INST_MAG, NAV_ACL_GYRO_WHO_AM_I, 1, &(InstancePtr->idData.mag));
    NAV_ReadRegister(InstancePtr, NAV_INST_ALT, NAV_ACL_GYRO_WHO_AM_I, 1, &(InstancePtr->idData.alt));
}
...
/* ------------------------------------------------------------ */
/*  void NAV_WriteRegister(PmodNAV* InstancePtr, uint8_t bInst, uint8_t bAddr, uint8_t bCntBytes, uint8_t *pData)
 *
**  Parameters:
**      InstancePtr - instance of PmodNAV
...
/* ------------------------------------------------------------ */
/*   void NAV_SetBitsInRegister(PmodNAV* InstancePtr, u8 bInst, u8 bRegAddr, u8 bMask, u8 bValue, u8 startBit)
**
**   Parameters:
**      InstancePtr - instance of PmodNAV
...
/* ------------------------------------------------------------ */
/*  u8 NAV_GetBitsInRegister(PmodNAV* InstancePtr, u8 bInst, u8 bRegAddr, u8 startBit, u8 noBits)
**
**  Parameters:
**      InstancePtr - instance of PmodNAV
**      bInst       - instrument selection for chip select: AG/MAG/ALT
...
/* ------------------------------------------------------------ */
/*   void NAV_SetRegisterBits(PmodNAV* InstancePtr, u8 bInst, u8 bRegAddr, u8 bMask, bool fValue)
**
**   Parameters:
**      InstancePtr - instance of PmodNAV
**      bInst       - instrument selection for chip select: AG/MAG/ALT
...
void NAV_ReadAccel(PmodNAV* InstancePtr, s16* X, s16* Y, s16*Z)
{
    ...
    NAV_ReadRegister(InstancePtr, NAV_INST_AG, NAV_ACL_OUT_X_L_XL, 6, (uint8_t *)rgwRegVals);
...
**
**  Description:
**      The function sets the appropriate g range bits in the NAV_ACL_CTRL_REG6_XL register.
**
*/
void NAV_SetRangeXL(PmodNAV* InstancePtr, u8 bRangeXL)
{
    InstancePtr->m_GRangeLSB = NAV_GetXLRangeLSB(bRangeXL);
    NAV_SetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_ACL_CTRL_REG6_XL, NAV_ACL_MSK_RANGE_XL, bRangeXL, 3);
}
...
**
**  Return Value:
**      uint8_t - returns the previously selected range from NAV_ACL_CTRL_REG6_XL register
...
**
**  Description:
**      The function reads the g range bits in the NAV_ACL_CTRL_REG6_XL register and computes the range to be provided to the user.
...
{
    u8 readRange, gRange;
    readRange = NAV_GetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_ACL_CTRL_REG6_XL, 3, 2);
    switch(readRange)
    {
        case NAV_ACL_PAR_XL_2G:
            gRange = 2;
...
**
**  Return Value:
**      uint8_t - returns the data available status in NAV_ALIAS_STATUS_REG register, for Accelerometer
**
**  Errors:
**      none
**
**  Description:
**      The function reads the NAV_ALIAS_STATUS_REG register and returns the data available bit in it(1 or 0)
**
*/
u8 NAV_DataAvailableXL(PmodNAV* InstancePtr)
{
...
**
**  Description:
**      The function configures the interrupt register NAV_ACL_INT_GEN_CFG_XL setting the interrupt generator.
...
**          NAV_ACL_MODE_INST_A     0 - aAccelerometer mode
**          NAV_ACL_GYRO_MODE_INST_AG    1 - Gyroscope
**  Return Value:
**      uint8_t - returns the content of the NAV_ACL_INT_GEN_SRC_XL or NAV_GYRO_INT_GEN_SRC_G, depending of bInstMode parameter
**
**  Errors:
**      none
**
...
void NAV_ReadGyro(PmodNAV* InstancePtr, s16* GX, s16* GY, s16* GZ)
{
    ...
    NAV_ReadRegister(InstancePtr, NAV_INST_AG, NAV_GYRO_OUT_X_L_G, 6, (u8 *)rgwRegVals);
    iGX_L = rgwRegVals[0];
    iGX_H = rgwRegVals[1];
    iGY_L = rgwRegVals[2];
    iGY_H = rgwRegVals[3];
...
**
**  Description:
**      The function sets the appropriate dps range bits in the NAV_GYRO_CTRL_REG1_G register.
**
*/
void NAV_SetRangeG(PmodNAV* InstancePtr, u8 bRangeG)
{
    InstancePtr->m_DPSRangeLSB = NAV_GetGRangeLSB(bRangeG);
    NAV_SetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_GYRO_CTRL_REG1_G, NAV_GYRO_MSK_RANGE_G, bRangeG, 3);
}
...
**
**  Return Value:
**      float - returns the previously selected range from NAV_GYRO_CTRL_REG1_G register
**      The return value is one of:
**          0       NAV_GYRO_PAR_G_245DPS    Parameter dps range: +/- 245dps
**          1       NAV_GYRO_PAR_G_500DPS    Parameter dps range: +/- 500dps
**          3       PAR_G_2000DPS   Parameter dps range: +/- 2000dps
...
**
**  Description:
**      The function reads the g range bits in the NAV_GYRO_CTRL_REG1_G register and computes the range to be provided to the user.
**      If the value is not a valid one, the default value for range is set
*/
float NAV_GetRangeG(PmodNAV* InstancePtr)
{
    u16 readRange, gRange;
    readRange = NAV_GetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_GYRO_CTRL_REG1_G, 3, 2);
    switch(readRange)
    {
        case NAV_GYRO_PAR_G_245DPS:
            gRange = 245;
...
**
**  Return Value:
**      uint8_t - returns the data available status in NAV_ALIAS_STATUS_REG register, for gyro
**
**  Errors:
**      none
**
**  Description:
**      The function reads the NAV_ALIAS_STATUS_REG register and returns the data available bit in it
**
*/
u8 NAV_DataAvailableG(PmodNAV* InstancePtr)
{
...
**          aoi 1  the interrupts are and-ed together
**      latch - parameter that sets or not the latch interrupt request
**          latch   0 nterrupt requests are not latched
...
**
**   Description:
**      The function configures the interrupt register NAV_GYRO_INT_GEN_CFG_G setting the interrupt generator.
**      Sets the interrupt events to or-ed or and-ed. Sets the interrupt event to be latched or not.
**
*/
void NAV_ConfigIntG(PmodNAV* InstancePtr, u8 bIntGen, bool aoi, bool latch)
...
/* ------------------------------------------------------------ */
/*  void NAV_SetIntThresholdG(PmodNAV* InstancePtr, float thVal, u16 axis, bool drCntMode, u8 intDuration, bool wait)
**
**  Parameters:
**      InstancePtr - instance of PmodNAV
...
void NAV_SetIntThresholdG(PmodNAV* InstancePtr, float thVal, u16 axis, bool drCntMode, u8 intDuration, bool wait)
{
    ...
    switch(axis)
    {
        ...
        //set the first bit, decrement or reset counter mode for interrupt
            ...
            NAV_WriteSPI(InstancePtr, NAV_INST_AG, NAV_GYRO_INT_GEN_THS_XH_G + (axis*2), buffer[0]);
            NAV_WriteSPI(InstancePtr, NAV_INST_AG, NAV_GYRO_INT_GEN_THS_XH_G + 1 + (axis*2), buffer[1]);
            break;
        case NAV_IMU_Y_AXIS:
            NAV_WriteSPI(InstancePtr, NAV_INST_AG, NAV_GYRO_INT_GEN_THS_XH_G + (axis*2), buffer[0]);
            NAV_WriteSPI(InstancePtr, NAV_INST_AG, NAV_GYRO_INT_GEN_THS_XH_G + 1 + (axis*2), buffer[1]);
...
/* ------------------------------------------------------------ */
/*  void NAV_ConfigInt(PmodNAV* InstancePtr, u8 bIntPin, u8 bIntGenMask, u8 bActiveType, u8 bOutputType)
**
**  Parameters:
**      InstancePtr - instance of PmodNAV
...
void NAV_ConfigInt(PmodNAV* InstancePtr, u8 bIntPin, u8 bIntGenMask, u8 bActiveType, u8 bOutputType)
{
    ...
    {
        NAV_SetRegisterBits(InstancePtr, NAV_INST_AG, NAV_ACL_GYRO_INT1_CTRL, bIntGenMask, 1);

    }
    else if (bIntPin == NAV_INT_PIN_2)
    {
        NAV_SetRegisterBits(InstancePtr, NAV_INST_AG, NAV_ACL_GYRO_INT2_CTRL, bIntGenMask, 1);
    }
...
void NAV_ReadMag(PmodNAV* InstancePtr, s16 *MagX, s16 *MagY, s16 *MagZ)
{
    ...
    while((status&0x08)==0);
    //reads the bytes using the incremeting address functionality of the device.
    NAV_ReadRegister(InstancePtr, NAV_INST_MAG, NAV_MAG_OUT_X_L_M, 6, (u8 *)rgwRegVals);
...
**
**  Return Values:
**      InstancePtr->MagData - the vector representation of the magnetometer reading in gauss
**
**  Errors:
**      none
**
**  Description:
**      This function is the main function used for magnetic field values reading, providing the 3 current magnetometer values in .
**      For each of the three values, converts the 16-bit raw value to the value expressed in , considering the currently selected Gauss range.
*/
void NAV_ReadMagGauss(PmodNAV* InstancePtr)
{
    s16 MagX, MagY, MagZ;
...
**
**  Description:
**      The function sets the appropriate gauss range bits in the NAV_MAG_CTRL_REG2_M register.
**
*/
void NAV_SetRangeMAG(PmodNAV* InstancePtr, u8 bRangeMAG)
{
    InstancePtr->m_GaussRangeLSB = NAV_GetMAGRangeLSB(bRangeMAG);
    NAV_SetBitsInRegister(InstancePtr, NAV_INST_MAG, NAV_MAG_CTRL_REG2_M, NAV_MAG_MSK_RANGE_MAG, bRangeMAG, 5);
}
...
**
**  Description:
**      The function reads the gauss range bits in the NAV_MAG_CTRL_REG2_M register and computes the range to be provided to the user.
**      If value is outside this range, the default value is set
**
*/
u8 NAV_GetRangeMAG(PmodNAV* InstancePtr)
{
    u8 readRange, gRange;
    readRange = NAV_GetBitsInRegister(InstancePtr, NAV_INST_MAG, NAV_MAG_CTRL_REG2_M, 5, 2);
    switch(readRange)
    {
        case NAV_MAG_PAR_MAG_4GAUSS:
            gRange = 4;
...
**
**  Return Value:
**       uint8_t - returns the data available status in NAV_MAG_STATUS_REG_M register, for the selected axis
**
**  Errors:
**      none
**
**  Description:
**      The function reads the NAV_MAG_STATUS_REG_M register and returns the data available bit in it(1 or 0), for each of the axes
**
*/
u8 NAV_DataAvailableMAG(PmodNAV* InstancePtr, u8 axis)
{
...
/* ------------------------------------------------------------ */
/*  void NAV_ConfigIntMAG(PmodNAV* InstancePtr, u8 bIntGen, u8 bActiveType, bool latch)
**
**  Parameters:
**      InstancePtr - instance of PmodNAV
...
void NAV_ConfigIntMAG(PmodNAV* InstancePtr, u8 bIntGen, u8 bActiveType, bool latch)
{
    ...
    if (bActiveType == NAV_PAR_INT_ACTIVEHIGH) config |= (1<<2);
...
void NAV_SetIntThresholdM(PmodNAV* InstancePtr, float thVal)
{
    u8 buffer[2];
    u16 bthVal;
    //converts the float value in gauss to raw magnetic field data to be written in NAV_MAG_INT_THS_L/NAV_MAG_INT_THS_H registers
...
**
**  Return Value:
**      uint8_t - returns the interrupt sources for magnetometer instrument, after reading NAV_MAG_INT_SRC_M register
**
**  Errors:
**      none
**
...
int NAV_ReadPressure(PmodNAV* InstancePtr)
{
    ...
    u8 rgwRegVals[3];
    int dataPress = 0;
    NAV_ReadRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_PRESS_OUT_XL, 3, (u8 *)rgwRegVals);
...
**
**   Return Values:
**      InstancePtr->tempC - the function returns the value of the read temperature in degrees Celsius via the PmodNAV device
**
**   Errors:
**      none
**
...
void NAV_ReadTempC(PmodNAV* InstancePtr)
{
    ...
    NAV_ReadRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_TEMP_OUT_L, 2, (u8 *)rgwRegVals);
    tempL = rgwRegVals[0];
    tempH = rgwRegVals[1];
...
**
**   Return Value:
**      uint8_t - returns the temperature available status in NAV_ALIAS_STATUS_REG register, for altimeter
**
**   Errors:
**      none
**
**   Description:
**      The function reads the NAV_ALIAS_STATUS_REG register and returns the temperature available bit in it(1 or 0)
**
*/
u8 NAV_TempAvailableALT(PmodNAV* InstancePtr)
{
...
**
**   Return Value:
**      uint8_t - returns the data available status in NAV_ALIAS_STATUS_REG register, for altimeter
**
**   Errors:
**		none
**
...
/* ------------------------------------------------------------ */
/*  void NAV_ConfigIntALT(PmodNAV* InstancePtr, u8 bIntGen, u8 bActiveType, u8 bOutputType,
** 	  					  u8 dataSignalVal, bool intEnable, bool latch, u8 intLevel)
**
**   Parameters:
**      InstancePtr - instance of PmodNAV
**      bIntGen - interrupt generator sources. It can be one of the following:
...
**
*/
void NAV_ConfigIntALT(PmodNAV* InstancePtr, u8 bIntGen, u8 bActiveType, u8 bOutputType, u8 dataSignalVal, bool intEnable, bool latch, u8 intLevel)
{
    //enable interrupts in NAV_ALT_CTRL_REG1 register
    NAV_SetRegisterBits(InstancePtr, NAV_INST_ALT, NAV_ALT_CTRL_REG1, NAV_ALT_MSK_DIFF_EN_ALT, intEnable);
...
**
**  Description:
**      The function gets the interrupt sources by reading the NAV_ALT_INT_SOURCE register
**
*/
u8 NAV_GetIntSrcALT(PmodNAV* InstancePtr)
{
...
**                  ----for Accelerometer
**                  0   NAV_ACL_ODR_XL_PWR_DWN  Parameter ODR value: power down the device
...
**                  ----for Gyro
**                  0   NAV_GYRO_ODR_G_PWR_DWN   Parameter ODR value: power down the device
...
void NAV_SetODR(PmodNAV* InstancePtr, u8 bInstMode, u8 odrVal)
{
    ...
    switch(bInstMode)
    {
        case NAV_ACL_MODE_INST_A:
            ...
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_ACL_CTRL_REG6_XL, NAV_ACL_MSK_ODR_XL, odrVal, 5);
            break;
        case NAV_ACL_GYRO_MODE_INST_AG:
            //set ODR for gyro and accel instruments when used together
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_GYRO_CTRL_REG1_G, NAV_GYRO_MSK_ODR_G, odrVal, 5);
            break;
        case NAV_MAG_MODE_INST_MAG:
            //set ODR for magnetometer instrument
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_MAG, NAV_ACL_CTRL_REG6_XL, NAV_MAG_MSK_ODR_MAG, odrVal, 3);
            break;
        case NAV_ALT_MODE_INST_ALT:
            //set ODR for altimeter instrument
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_CTRL_REG1, NAV_ALT_MSK_ODR_ALT, odrVal, 4);
            break;
        default:
            break;
    }
...
u8 NAV_GetODRRaw(PmodNAV* InstancePtr, u8 bInstMode)
{
    ...
    {
        case NAV_ACL_MODE_INST_A:
            getODR = NAV_GetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_ACL_CTRL_REG6_XL, 5, 3);
            break;
        case NAV_ACL_GYRO_MODE_INST_AG:
            getODR = NAV_GetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_GYRO_CTRL_REG1_G, 5, 3);
            break;
        case NAV_MAG_MODE_INST_MAG:
            getODR = NAV_GetBitsInRegister(InstancePtr, NAV_INST_MAG, NAV_ACL_CTRL_REG6_XL, 3, 3);
            break;
        case NAV_ALT_MODE_INST_ALT:
            getODR = NAV_GetBitsInRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_CTRL_REG1, 4, 3);
            break;
        default:
            break;
    }
...
**      none
**   Description:
**      The function enables or disables FIFO by writing FIFO_EN bit in NAV_IMU_CTRL_REG9 register for Accel/Gyro, or NAV_ALT_CTRL_REG2 for Altimeter instrument
**
*/
void NAV_FIFOEnable(PmodNAV* InstancePtr, u8 bInst, bool fEnable)
{
...
/* ------------------------------------------------------------ */
/*  void NAV_SetFIFO(PmodNAV* InstancePtr, u8 bInst, u8 parFIFOMode, u8 FIFOThreshold)
**
**  Parameters:
**     	InstancePtr - instance of PmodNAV
...
**                  0   NAV_ACL_GYRO_FIFO_MODE_XL_G_BYPASS           Parameter FIFO mode value: bypass
**                  1   NAV_ACL_GYRO_FIFO_MODE_XL_G_FIFO             Parameter FIFO mode value: FIFO normal
**                  3   NAV_ACL_GYRO_FIFO_MODE_XL_G_CONTINUOUS_FIFO  Parameter FIFO mode value: continuous to FIFO
**                  4   NAV_ACL_GYRO_FIFO_MODE_XL_G_BYPASS_CONTINUOUS        Parameter FIFO mode value: bypass to continuous
**                  6   NAV_ACL_GYRO_FIFO_MODE_XL_G_CONTINUOUS       Parameter FIFO mode value: continuous
**                  ----for Altimeter instrument
**                  0   NAV_ALT_FIFO_MODE_ALT_BYPASS            Parameter FIFO mode value: bypass
**                  1   NAV_ALT_FIFO_MODE_ALT_FIFO              Parameter FIFO mode value: FIFO normal
**                  2   NAV_ALT_FIFO_MODE_ALT_STREAM            Parameter FIFO mode value: stream mode
**                  3   NAV_ALT_FIFO_MODE_ALT_STREAM_TO_FIFO    Parameter FIFO mode value: stream to fifo
**                  4   NAV_ALT_FIFO_MODE_ALT_BYPASS_TO_STREAM  Parameter FIFO mode value: bypass to stream
**                  6   NAV_ALT_FIFO_MODE_ALT_MEAN              Parameter FIFO mode value: mean mode
**                  7   NAV_ALT_FIFO_MODE_ALT_BYPASS_TO_FIFO    Parameter FIFO mode value: bypass to fifo mode
...
**                  ----for Altimeter instrument
**                  0   NAV_ALT_FIFO_MODE_MEAN_ALT_2SAMPLES     Parameter FIFO mean mode value: 2 samples
**                  1   NAV_ALT_FIFO_MODE_MEAN_ALT_4SAMPLES     Parameter FIFO mean mode value: 4 samples
**                  2   NAV_ALT_FIFO_MODE_MEAN_ALT_8SAMPLES     Parameter FIFO mean mode value: 8 samples
**                  3   NAV_ALT_FIFO_MODE_MEAN_ALT_16SAMPLES    Parameter FIFO mean mode value: 16 samples
**                  4   NAV_ALT_FIFO_MODE_MEAN_ALT_32SAMPLES    Parameter FIFO mean mode value: 32 samples
**   Return Value:
**      none
**
**   Errors:
...
void NAV_SetFIFO(PmodNAV* InstancePtr, u8 bInst, u8 parFIFOMode, u8 FIFOThreshold)
{
    ...
    {
        case NAV_INST_AG:
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_IMU_FIFO_CTRL, NAV_IMU_MSK_FIFO_CTL_MODE, parFIFOMode, 5);
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_IMU_FIFO_CTRL, NAV_IMU_MSK_FIFO_THS, FIFOThreshold, 0);
            break;
        case NAV_INST_ALT:
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_FIFO_CTRL, NAV_ALT_MSK_FIFO_CTL_MODE, parFIFOMode, 5);
            NAV_SetBitsInRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_FIFO_CTRL, NAV_ALT_MSK_FIFO_THS, FIFOThreshold, 0);
            break;
        default:
            break;
    }
...
u8 NAV_GetFIFOMode(PmodNAV* InstancePtr, u8 bInst)
{
    ...
    {
        case NAV_INST_AG:
            getFIFO = NAV_GetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_IMU_FIFO_CTRL, 5, 3);
            break;
        case NAV_INST_ALT:
            getFIFO = NAV_GetBitsInRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_FIFO_CTRL, 5, 3);
            break;
        default:
            break;
    }
...
u8 NAV_GetFIFOThs(PmodNAV* InstancePtr, u8 bInst)
{
    ...
    {
        case NAV_INST_AG:
            bFifoThs = NAV_GetBitsInRegister(InstancePtr, NAV_INST_AG, NAV_IMU_FIFO_CTRL, 0, 5);
            break;
        case NAV_INST_ALT:
            bFifoThs = NAV_GetBitsInRegister(InstancePtr, NAV_INST_ALT, NAV_ALT_FIFO_CTRL, 0, 5);
            break;
        default:
            break;
    }
...
**
**  Return Value:
**     uint8_t - returns the FIFO status NAV_ALT_FIFO_STATUS or NAV_IMU_FIFO_SRC register, depending on the selected instrument
**
**  Errors:
**      none
**
**  Description:
**      The function reads the NAV_IMU_FIFO_SRC register for Accelerometer and Gyroscope and NAV_ALT_FIFO_STATUS register for Altimeter instrument
**      and returns their value.
**
*/
u8 NAV_GetFIFOStatus(PmodNAV* InstancePtr, u8 bInst)
